...
local preview_win_big = function(lines)
    local buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, lines)
    ...
    local win_lines = vim.api.nvim_get_option("lines")
    local win_cols = vim.api.nvim_get_option("columns")
    local width = math.floor(win_cols - 20)
...
local debug_big = function(result)
    -- Use vim.inspect for pretty printing
    local inspected = vim.inspect(result, { depth = 10 })
...
vim.keymap.set("n", "<leader>a", persistent_signature_help, { desc = "Persistent Signature Help" })
